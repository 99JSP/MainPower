...
function server.setPlayerInventory(player, data)
	...
	if not data then
		...
		data = db.loadPlayer(player.citizenid)
	end

	-- print("passed data fill?")
	...
	-- print("CREATING?????")
	local new__name = "" .. player.firstname .. " " .. player.lastname .. ""
	player.id = tonumber(player.id)
	local inv = Inventory.Create(player.PlayerId, new__name, 'player', shared.playerslots, totalWeight, shared.playerweight, player.citizenid, inventory)
	-- print("Passed inv create? and source = " .. player.PlayerId .. " soruc")

	if inv then
		inv.player = server.setPlayerData(player)
		inv.player.ped = GetPlayerPed(player.PlayerId)

		if server.syncInventory then server.syncInventory(inv) end
		TriggerClientEvent('ox_inventory:setPlayerInventory', player.PlayerId, Inventory.Drops, inventory, totalWeight, inv.player)
	end
end
