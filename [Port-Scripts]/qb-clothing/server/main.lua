...
AddEventHandler('qb-clothing:saveSkin', function(model, skin)
    ...
    if model ~= nil and skin ~= nil then
        ...
        MySQL.query('DELETE FROM playerskins WHERE citizenid = ?', { Player.Data.citizenid }, function()
...
AddEventHandler('qb-clothes:loadPlayerSkin', function()
    local src = source
    local Player = MP.Functions.GetPlayer(src)
    local result = MySQL.query.await('SELECT * FROM playerskins WHERE citizenid = ? AND active = ?', { Player.Data.citizenid, 1 })
...
AddEventHandler("qb-clothes:saveOutfit", function(outfitName, model, skinData)
    ...
    if model ~= nil and skinData ~= nil then
        ...
        MySQL.insert('INSERT INTO player_outfits (citizenid, outfitname, model, skin, outfitId) VALUES (?, ?, ?, ?, ?)', {
            ...
            local result = MySQL.query.await('SELECT * FROM player_outfits WHERE citizenid = ?', { Player.Data.citizenid })
            if result[1] ~= nil then
...
MP.Functions.RegisterServerCallback('qb-clothing:server:getOutfits', function(source, cb)
    local src = source
    local Player = MP.Functions.GetPlayer(src)
    local anusVal = {}
