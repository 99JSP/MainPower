...
RegisterNetEvent('MP:Client:OnPlayerLoaded', function()
    ...
    PlayerData = MP.Functions.GetPlayerData(source)
end)
...
RegisterNetEvent('qb-clothing:client:getOutfits', function(requiredJob, gradeLevel)
    local gender = "Male"
    if MP.Functions.GetPlayerData().sex == "Female" then gender = "female" end
...
if Config.UseTarget then
    ...
    CreateThread(function()
        for k, v in pairs(Config.Stores) do
            ...
            exports.ox_target:addBoxZone(v.shopType .. k, v.coords, v.length, v.width, {
                name = v.shopType .. k,
                debugPoly = false,
                minZ = v.coords.z-1,
                maxZ = v.coords.z+1,
        ...
        for k, v in pairs(Config.ClothingRooms) do
            ...
            exports.ox_target:addBoxZone('clothing_' .. v.requiredJob .. k, v.coords, v.length, v.width, {
        ...
        clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                ...
                if zoneName == 'surgeon' then
					lib.showTextUI('[E] - Plastic Surgery', {
						position = "top-center",
						icon = 'hand',
						style = {
							borderRadius = 0,
                ...
                elseif zoneName == 'clothing' then
					lib.showTextUI('[E] - Clothing Shop', {
						position = "top-center",
						icon = 'hand',
						style = {
							borderRadius = 0,
...
RegisterNUICallback('removeOutfit', function(data, cb)
	...
	exports['MP-Elements']:Noti(1, "You have deleted your"..data.outfitName.." outfit!", 6000)
    cb('ok')
end)
...
RegisterNUICallback('setCurrentPed', function(data, cb)
    local playerData = MP.Functions.GetPlayerData(source)
	print("testing ... " .. playerData.sex .. " is data?")
...
AddEventHandler('qb-clothing:client:loadOutfit', function(oData)
    ...
    if oData.outfitName ~= nil then
		exports['MP-Elements']:Noti(1, "You have chosen "..oData.outfitName.."! Press Confirm to confirm outfit.", 6000)
    end
end)
...
RegisterCommand("sex", function(source)
	local sex = MP.Functions.GetPlayerData(source).sex
	print(sex)
end)
...
function reloadSkin(health)
    ...
    local gender = MP.Functions.GetPlayerData(source).sex
    local maxhealth = GetEntityMaxHealth(PlayerPedId())

    if gender == "Female" then -- Gender is ONE for FEMALE
